 ...
 */
function nexus_custom_controls( $wp_customize ) {
	// Custom control for dropdown category multiple select.
	class Nexus_Multi_Categories_Control extends WP_Customize_Control {
		...
		public function render_content() {
			$dropdown = wp_dropdown_categories(
				array(
					...
					'hide_if_empty'    => false,
					'show_option_all'  => esc_html__( 'All Categories', 'nexus' ),
				)
			);
			...
			echo '<p class="description">' . esc_html__( 'Hold down the Ctrl (windows) / Command (Mac) button to select multiple options.', 'nexus' ) . '</p>';
		}
	}

	// Custom control for any note, use label as output description.
	class Nexus_Note_Control extends WP_Customize_Control {
...
add_action( 'customize_register', 'nexus_custom_controls', 1 );
