...
if ( ! function_exists( 'nexus_breadcrumb' ) ) :
	function nexus_breadcrumb() {
		/* === OPTIONS === */
		$text['home']     = esc_html__( 'Home', 'nexus' ); // text for the 'Home' link
		/* translators: 1: before text/html, 2: after text/html. */
		$text['category'] = esc_html__( '%1$s Archive for %2$s', 'nexus' ); // text for a category page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['search']   = esc_html__( '%1$sSearch results for: %2$s', 'nexus' ); // text for a search results page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['tag']      = esc_html__( '%1$sPosts tagged %2$s', 'nexus' ); // text for a tag page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['author']   = esc_html__( '%1$sView all posts by %2$s', 'nexus' ); // text for an author page
		$text['404']      = esc_html__( 'Error 404', 'nexus' ); // text for the 404 page
		...
		$link       = $linkBefore . '<a' . $linkAttr . ' href="%1$s">%2$s</a>' . $linkAfter;
			...
			if ( is_category() ) {
				...
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'nexus' ) ) ) ), esc_html( get_the_time( __( 'Y', 'nexus' ) ) ) ) ; // WPCS: XSS OK.

				echo sprintf( $link, esc_url( get_month_link( get_the_time( __( 'Y', 'nexus' ) ), get_the_time( __( 'n', 'nexus' ) ) ) ), esc_html( get_the_time( __( 'F', 'nexus' ) ) ) ); // WPCS: XSS OK.

				echo $before . esc_html( get_the_time( __( 'd', 'nexus' ) ) ) . $after; // WPCS: XSS OK.

			} elseif ( is_month() ) {
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'nexus' ) ) ) ), esc_html( get_the_time( __( 'Y', 'nexus' ) ) ) ) ; // WPCS: XSS OK.

				echo $before . esc_html( get_the_time( __( 'F', 'nexus' ) ) ) . $after; // WPCS: XSS OK.


			} elseif ( is_year() ) {
				echo $before . esc_html( get_the_time( __( 'Y', 'nexus' ) ) ) . $after; // WPCS: XSS OK.
			...
			if ( get_query_var( 'paged' ) ) {
				...
				echo sprintf( esc_html__( 'Page %s', 'nexus' ), absint( max( $paged, $page ) ) );
